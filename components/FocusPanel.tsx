...
const FocusPanel: React.FC<Props> = ({ focus }) => {
  // Defensive checks
  const chainOfThought = focus?.chain_of_thought || [];
  const pendingTasks = focus?.pending_tasks || [];
  const currentObjective = focus?.current_objective || "No objective set";
  const lastUpdated = focus?.last_updated ? new Date(focus.last_updated).toLocaleString() : "Unknown";
  ...
  return (
    <div className="flex flex-col h-full space-y-6 overflow-y-auto pr-2 custom-scrollbar">
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-lg">
        ...
        <p className="text-zinc-100 font-medium text-lg">{currentObjective}</p>
