...
const MemoryPanel: React.FC<Props> = ({ memory }) => {
  // Defensive checks: ensure arrays exist before rendering
  const activeProjects = memory?.active_projects || [];
  const confidenceMetrics = memory?.confidence_metrics || [];
  const learnedTruths = memory?.learned_truths || [];
  const coreInstructions = memory?.core_instructions || [];
  const knowledgeGraph = memory?.knowledge_graph || { nodes: [], edges: [] };
    ...
    <div className="flex flex-col h-full space-y-6 overflow-y-auto pr-2 custom-scrollbar">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3">
            {activeProjects.length === 0 && <p className="text-zinc-600 text-sm italic">No active projects.</p>}
      ...
      <div>
        ...
        <ul className="space-y-2">
            {learnedTruths.map((truth, idx) => (
                <li key={idx} className="text-zinc-400 text-sm flex items-start gap-2">
                    <span className="text-zinc-600 mt-1">•</span>
                    {truth}
                </li>
            ))}
            {learnedTruths.length === 0 && <li className="text-zinc-600 text-sm italic">No truths recorded.</li>}
        </ul>
      </div>
       ...
       <div>
        ...
        <div className="bg-zinc-900/50 p-3 rounded border border-zinc-800 font-mono text-xs text-zinc-400">
            {coreInstructions.map((inst, i) => (
                <div key={i} className="mb-1 text-green-500/80">
                    &gt; {inst}
                </div>
            ))}
            {coreInstructions.length === 0 && <div className="text-zinc-600 italic">No instructions loaded.</div>}
        </div>
      </div>
